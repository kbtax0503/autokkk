...
1. 앱 실행 → **서버 주소**(예: `https://www.dawoncredit.com/kakao`)와 **폰 토큰**(서버 `KAKAO_BRIDGE_PHONE_TOKEN` 값) 입력 → **설정 저장**.
...
- 서버는 외부 HTTPS(`https://www.dawoncredit.com/kakao` → tax nginx → NAS:8195)로 공개돼, 폰이 사무실 WiFi 밖(모바일데이터)에서도 도달 가능. LAN 직결(`http://192.168.45.101:8195`)도 가능(평문 허용).
